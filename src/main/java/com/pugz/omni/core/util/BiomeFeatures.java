...
public class BiomeFeatures {
    ...
    public static void addScatteredBlock(BiomeGenerationSettingsBuilder biome, BlockState state, Set<Block> placers, int freq, int chance) {
        biome.getFeatures(GenerationStage.Decoration.VEGETAL_DECORATION).add(() -> Feature.RANDOM_PATCH.withConfiguration(scatteredBlockConfig(state, placers, freq)).func_242730_a(FeatureSpread.func_242253_a(-1, 4)).withPlacement(Features.Placements.HEIGHTMAP_PLACEMENT).func_242731_b(5).func_242729_a(chance));
    }
    ...
    public static boolean hasFeature(BiomeGenerationSettingsBuilder biome, GenerationStage.Decoration stage, @Nonnull ConfiguredFeature<?, ?> feature) {
        for (Supplier<ConfiguredFeature<?, ?>> configuredFeature : biome.getFeatures(stage)) {
            return configuredFeature.get() == feature;
        }

        return false;
    }

    public static void removeFeature(BiomeGenerationSettingsBuilder biome, GenerationStage.Decoration stage, @Nonnull ConfiguredFeature<?, ?> feature) {
